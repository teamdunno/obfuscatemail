...
export type Nullish<T> =
  | { [K in keyof T]+?: T[K] extends object ? Nullish<T[K]> : T[K]}
  | Undef;
function getOptionsProto<T extends object>(
  options: Nullish<T>,
  def: T,
  fromDes?: string,
): T {
  ...
  for (const [_k, v] of Object.entries(options!)) {
    ...
    if (
      typeof resDyn[_k] !== typeof v ||
      (resDyn[_k] instanceof Array) !== (v instanceof Array)
    ) {
      throw new TypeError(
        `Object ${
          fromDes ? `${fromDes}.` : ''
        }${_k} type needs to be same as Options.${
          fromDes ? `${fromDes}.` : ''
        }${_k}`,
      );
    }
    ...
    if (typeof v === 'object' && !(v instanceof Array)) {
      type Nested = T[typeof k] extends Record<string, unknown> ? T[typeof k]
        : never;
      res[k] = getOptionsProto<Nested>(
        v,
        res[k] as Nested,
        fromDes ? `${fromDes}.${String(k)}` : String(k),
      );
    } else {
